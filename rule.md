...
- 场景：$$X_t=\{O_{t,1},\dots,O_{t,M_t}\},\ M_t\in\{2,3,4,5\}$$
...
- **R1-10**：$$r_{t+1}=k r_t,\ R_{t+1}=R_t\text{Rot}(\hat{u},\theta)$$
- **R1-11**：形状/尺度交替互换
- **R1-12**：移动方向与主轴一致
- **R1-13**：远离锚点变大，靠近锚点变小
- **R1-14**：镜像位置下尺寸一增一减
...
- **R2-8**：共同旋转，夹角恒定
- **R2-9**：$$r_{t+1}=r_t\odot s,\ s_{axis}=k,\ s_{\text{others}}=\tfrac{1}{\sqrt{k}}$$
...
## 删除的规则
R4 结构与拓扑类别及 `C04/C05/C06/C07` 全部移除，规则总数为 33；不再提供 `r4-only`、`all-minus-r4` 等模式。
