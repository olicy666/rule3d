...
## R3 多物体构型（R3-1–R3-11）
- **R3-1**：$$\sum_i \text{size}(i)=C,\ \text{size}_i^{t+1}=\text{size}_i^t+\Delta_i,\ \sum_i \Delta_i=0$$（允许 $\Delta_i=0$）
...
- **R3-6**：多边形顶点移动但面积保持恒定（3–6 个顶点）
- **R3-7**：多对象位置轮换，顺/逆时针按步长轮换（直线/三角形/矩形/五边形）
