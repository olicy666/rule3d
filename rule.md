...
- **R1-7**：$$r_{t+1}=k r_t,\ R_{t+1}=R_t\text{Rot}(\hat{u},\theta)$$（复合位姿缩放）
- **R1-8**：移动方向与主轴一致（主轴对齐位移）
- **R1-9**：密度越大缩放越大，密度越小缩放越小（密度驱动缩放）
- **R1-10**：距离越近尺寸变化越剧烈（距离尺寸倒数）
- **R1-11**：不同形状数量按等差增加（几何体个数变化）

## R2 几何交互推理（R2-1–R2-16）
- **R2-1**：$$\text{cent}(S_t)=\text{const},\ r_{t+1}=k r_t$$（质心守恒缩放）
- **R2-2**：$$\text{size}(i)+\text{size}(j)=C$$，一增一减
- **R2-3**：尺度/姿态交替互换（形状固定）
- **R2-4**：距离越近密度变化越剧烈（距离密度倒数）
- **R2-5**：$$\text{dist}_2=k\text{dist}_1,\ \text{dist}_3=k\text{dist}_2$$
- **R2-6**：$$r_{t+1}=r_t\odot s,\ s_{axis}=k,\ s_{\text{others}}=\tfrac{1}{\sqrt{k}}$$
- **R2-7**：方向旋转等差，距离恒定
- **R2-8**：$$\rho_{t+1}-\rho_t=\Delta,\ \rho_t\in(0,1)$$
- **R2-9**：$$\text{ang}_3=2\text{ang}_2-\text{ang}_1$$
- **R2-10**：共同旋转，夹角恒定
- **R2-11**：加速旋转，后续旋转幅度增加
- **R2-12**：行星公转，不同物体角速度不同
- **R2-13**：易位姿态转换，按形状延续姿态变化
- **R2-14**：易位尺寸转换，按形状延续尺寸变化
- **R2-15**：易位密度转换，按形状延续密度变化
- **R2-16**：$$(r_x,r_y,r_z)\to(r_y,r_z,r_x)\to(r_z,r_x,r_y)$$

## R3 结构组合推理（R3-1,R3-2,R3-3,R3-4,R3-5,R3-7–R3-11）
- **R3-1**：所有几何体沿螺旋上升，角度步长一致且高度等差递增
- **R3-2**：$$\text{ord}_x(S_t)$$ 按固定置换循环
- **R3-3**：6 个几何体直线阵型整体等角度旋转并延续
- **R3-4**：密度循环吞噬，高密度几何体按顺/逆时针吞噬相邻几何体（6→5→4）
- **R3-5**：尺寸循环吞噬，大几何体按顺/逆时针吞噬相邻几何体（6→5→4）
